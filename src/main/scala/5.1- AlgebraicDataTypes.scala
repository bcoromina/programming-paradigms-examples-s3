@main def AlgebraicDataTypes(): Unit =
  ...
  val u = User(1, "Bernat", "bcoromina@gmail.com")
  u.copy(name = "Alice") // copy method is generated by the compiler for case classes.
  // It allows us to create a new instance of the case class with some fields modified.

  // State transition with Immutable classes
  ...
  val counter2 = counter1.increment()


  class SomeClass(val value: Int, val name: String)

  object SomeClass:
    def apply(value: Int, s: String): SomeClass = new SomeClass(value,s)
    def unapply(s: SomeClass): Option[(Int,String)] = Some((s.value, s.name))
  // unapply is used for pattern matching.
  // It returns an Option of the values we want to extract from the class.
